...
| 대표그림 (일기) | 목록(타임라인)엔 이미지가 없는 디자인. `cover`를 지정하면 글 본문 상단에 표시된다 |
| 지운 이미지 정리 | Typora에서 이미지를 지워도 파일은 남는다 → 루트의 `clean-images.bat` 더블클릭: 어느 글에서도 참조 안 하는 이미지를 `_to_delete\orphan-images`로 모아준다 (확인 후 폴더째 삭제) |
